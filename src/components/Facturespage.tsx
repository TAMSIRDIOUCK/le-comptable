...
export default function FacturesPage({ userId, profile, onNouvelleFacture, openFactureId }: FacturesPageProps) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from('factures_comptable').select('*').eq('user_id', userId).order('created_at', { ascending: false });
    setFactures(data || []);
    setLoading(false);
  }, [userId]);
  ...
  if (viewId) return (
    <FactureView factureId={viewId} profile={profile} onBack={() => { setViewId(null); load(); }} />
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/40 text-sm mt-0.5">{factures.length} facture{factures.length > 1 ? 's' : ''} au total</p>
        </div>
        <button onClick={onNouvelleFacture}
        ...
        <div className="flex gap-2 flex-wrap">
          {STATUTS.map(s => (
            <button key={s} onClick={() => setStatutFilter(s)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${statutFilter === s ? 'bg-emerald-500 text-white' : 'bg-[#0d1627] text-white/40 hover:text-white border border-white/10'}`}>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map(f => {
            ...
            const Icon = cfg.icon;
            return (
              <div key={f.id} className="bg-[#0d1627] border border-white/5 hover:border-white/10 rounded-xl px-5 py-4 flex items-center gap-4 transition group cursor-pointer"
                ...
                {/* Montant */}
                <div className="text-white font-black text-base shrink-0 ml-2">{fmt(f.total_ttc)} F</div>
                {/* Actions */}
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition shrink-0" onClick={e => e.stopPropagation()}>
                  <button onClick={() => setViewId(f.id)} className="text-white/30 hover:text-white transition p-1.5 rounded-lg hover:bg-white/5">
                    <Eye className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(f.id, f.numero)} className="text-white/30 hover:text-red-400 transition p-1.5 rounded-lg hover:bg-red-500/5">
