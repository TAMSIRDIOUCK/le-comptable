...
export default function FactureView({ factureId, profile, onBack }: FactureViewProps) {
  ...
  const updateStatut = async (statut: Facture['statut']) => {
    await supabase.from('factures_comptable')
      .update({ statut, updated_at: new Date().toISOString() }).eq('id', factureId);
  ...
  /* ── Styles inline partagés pour le rendu React ET l'impression ── */
  const s = {
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '25px' }}>
              <thead>
                <tr style={{ background: '#0a0f1e' }}>
                  {[
                    { label: 'Désignation',      align: 'left',  w: '30%' },
                    { label: 'Couleur / Format', align: 'left',  w: '20%' },
                    { label: 'Qté (m²)',         align: 'right', w: '12%' },
                    { label: 'Prix/m² (F CFA)',  align: 'right', w: '18%' },
                    { label: 'Total (F CFA)',    align: 'right', w: '20%' },
                  ].map(h => (
                    <th key={h.label} style={{ padding: '12px 14px', textAlign: h.align as any, fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'white', fontWeight: 700, width: h.w }}>
                      {h.label}
                    </th>
                  ))}
                </tr>
              ...
              <div style={{ width: '280px' }}>
                ...
                {facture.remise_montant > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '13px', color: '#d97706' }}>
                    <span>Remise{facture.remise_pct > 0 ? ` (${facture.remise_pct}%)` : ''}</span>
